...
- **Close obstacle check:** when both the character and destination can be sampled onto NavMesh, the close-range reached test also requires a clear `NavMesh.Raycast` segment between them. A railing, wall, or switchback divider therefore cannot count as reached merely because the target is within the horizontal stopping radius on the other side.
...
- `CornerReachDistance` uses full 3D distance. Ramp and stair routes can contain vertically separated corners with nearly identical X/Z coordinates, especially on switchback towers. Horizontal-only advancement can skip those upper corners while the character is still below them, discard the valid path, and make the movement behavior fall back to walking directly at the elevated target.
